...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def cost_function(self, node_current):
        ...
        #Comment out euclidean  distance and uncomment out the Octile distance obove
    ...
    def execute_search(self,w,time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        while len(open_list) > 0:
            ...
            for v in open_list:
                ...
                if n == None or self.evaluation_function(v,w) < self.evaluation_function(n,w):
                    n = v;

            if n == None:
                print('Nothing in open list algorithm failed!')
                return None

            # parse through all successors available starting from the initial_node
            for primitive_successor in n.get_successors():

                collision_flag, child = self.take_step(successor=primitive_successor, node_current=n)
                ...
                if child not in open_list and child not in closed_list:
                    ...
                    if self.cost_function(child) > self.cost_function(n):
